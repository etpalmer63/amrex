...
In ``amrex/Tools/Plotfile``, type  ``make``
to generate an executable.  To run the executable, execute ``./fextract.gnu.ex``. Running ``./fextract.gnu.ex`` with no
arguments will print the full command line and all the available options.
It is possible to select the axis (``-d`` flag) where the data is collected. By default the axis is taken at the center of the domain.
A generic ASCII file is generated by default, which contains many details of the simulation.
Data can be exported in a raw csv file with the command ``-csv``.
